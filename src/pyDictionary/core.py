...
class Core():
    ...
    def __init__(self) -> None:
        ...
        self.instruction = Align.center(Padding("[cyan]space[/cyan]: to activate voice"),vertical="bottom")
...
def find(state):
    ...
    split_entry_text = state.current_entry_text.split(" ")
    last_word = split_entry_text[-1]
    ...
    if word and definition:
        ...
        # Build the panel
        panel = build_definition_panel(word, formatted_definition, state)
        table.add_row(panel)
    else:
        # Build not found message
