...
def getTable(ui_state:"Core"):
    ...
    def switch_to_synonyms():
        """Switch to dictionary screen with optional additional setup"""
        ui_state.current_screen = AppState.DICTIONARY
        ui_state.command = ui_state.PRIMARY_KEY_WORD_MAPPING["synonym"]
        ui_state.dictionary_state = DictionaryState.SYNONYM
    def switch_to_rhymes():
        """Switch to dictionary screen with optional additional setup"""
        ui_state.current_screen = AppState.DICTIONARY
        ui_state.command = ui_state.PRIMARY_KEY_WORD_MAPPING["rhyming_words"]
        ui_state.dictionary_state = DictionaryState.RHYMING_WORDS


    def switch_to_definitions():
        """Switch to dictionary screen with optional additional setup"""
        ui_state.current_screen = AppState.DICTIONARY
        ui_state.command = ui_state.PRIMARY_KEY_WORD_MAPPING["search_by_definition"]
        ui_state.dictionary_state = DictionaryState.DEFINITION

    switch_to_games =  lambda: setattr(ui_state, 'current_screen', AppState.GAMES_SELECTION)

    switch_to_help= lambda : setattr(ui_state, 'current_screen', AppState.HELP)
    options = {
        ...
        "help": switch_to_help,
        "exit": lambda: setattr(ui_state, 'running', False)
    }
